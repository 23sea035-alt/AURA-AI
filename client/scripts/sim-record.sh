...
# frames so transient UI can be reviewed frame-by-frame: keyboard transitions,
# screen animations, splash handoffs — things discrete screenshots skip over.
# Ported from the Amibroke repo's tools/sim-record.sh (fully generic, no
# project-specific paths beyond the bundle id you pass in).
#
# Usage:
#   scripts/sim-record.sh <label> <seconds> [bundle-id-to-cold-launch] [fps]
#
# Examples:
#   scripts/sim-record.sh keyboard 4 com.aura.ai.companion 30   # cold-launch + record 4s @30fps
#   scripts/sim-record.sh swipe 6                                # record whatever's on screen
#
# Output:
