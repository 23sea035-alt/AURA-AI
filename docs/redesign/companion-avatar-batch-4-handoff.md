...
1. Generate on magenta, iterate ≥3× until the face + framing are right (don't settle).
2. **Key out the magenta** with the tool (deterministic, no blur):
   ...
   ```
   It removes the `#FF00FF` background (border-connected, with a 1px de-fringe) and resizes to 1254². Manual
   fallback: paint.net → Magic Wand on the magenta → delete → export 1254².
