...
- **Voice — units built, NOT wired end-to-end.** The stack is **binary WebSocket frames** + **Inworld TTS 2** (`inworld-tts-2`) + **Groq STT** (`whisper-large-v3-turbo`, batch); LiveKit / Cartesia / Deepgram are fully removed (no such env vars or imports remain). The **Inworld TTS and Groq STT clients are real, non-stub implementations**, but the end-to-end audio path is **not connected**: the WebSocket handler drops inbound binary/audio frames (no STT→ChatSession→TTS wiring), `VoiceSession`/`makeVoiceAdapter`/interruption are never instantiated at runtime, and **only `GET /api/voice/limits` exists** — `POST /api/voice/start` + `/stop` are specced (D13) but **not yet implemented**, so `recordVoiceUsage`/`checkCallDurationLimit` are never called and metering is currently inert. `/api/voice/token` + `/api/voice/tts` were deleted per D13. Schema (`voice_usage`), limits, and the `whisper`/`inworld-tts-2` model wiring are in place for when the path is finished.
...
- **Decision:** Ship realtime voice. Audio transport via **binary WebSocket frames** on the existing chat connection (see §4); **Inworld TTS 2** (`inworld-tts-2`) for TTS (migrated from Cartesia); **Groq STT** (`whisper-large-v3-turbo`, batch mode) for STT. Session lifecycle managed by REST endpoints (`GET /api/voice/limits`, `POST /api/voice/start`, `POST /api/voice/stop` — **only `/limits` implemented as of 2026-07-02**). Usage metered into `voice_usage`, gated by `VOICE_DAILY_LIMIT_SECONDS` / `VOICE_CALL_MAX_DURATION_SECONDS` (premium variants in `@aura/shared`).
