...
App name: Aura (resolved 2026-06-24). See planning/app-name-research.md for the full availability audit.
...
Config is the root ../vitest.config.ts; test files live in ../server/src/__tests__/. Contract tests run against a real Postgres via PGlite (in-memory) — setup.ts boots it and applies Drizzle migrations, so they exercise the actual schema. LLM providers are mocked. This is what ../.github/workflows/ci.yml enforces on every push/PR to main (build + typecheck + lint + test).
...
These call the Groq LLM provider for real and are not part of pnpm test / CI. The moderation runner enforces FN = 0 on safety-critical categories (sexual/minors, self-harm, injection) — exits 1 if any slip through. See TODO.md §6 for the iteration loop instructions.
...
Memory pipeline (async consolidation)	specs/memory-pipeline.md
Generation pipeline (prompt assembly)	specs/generation-pipeline.md
3-tier testing strategy	testing/test-harness.md
Eval safety rubric + report format	testing/eval-safety-rubric.md · testing/eval-report-layout.md
Phase A–F forensic history	planning/backend-fixlist-v1.md
Post-launch roadmap	planning/post-v1.0-roadmap.md
Frontend items (client owner only)	planning/frontend-todo.md
Production-readiness audit (2026-06-29)	audit/backend-audit-2026-06.md
Compliance drafts (do not publish without counsel)	compliance/
Ownership
Role	Person	Scope
Backend / server	Coworker (Replit)	Work TODO.md top-to-bottom. Does NOT build the frontend — append client-affecting contract changes to planning/frontend-todo.md under "Backend-driven items."
...
This workspace is the server only — the iOS client requires Mac + Xcode + EAS and is not buildable on Replit. The Agent reads ../replit.md + ../AGENTS.md each session; both point here and to TODO.md. Work TODO.md in order, one item at a time; after each — pnpm build && pnpm typecheck && pnpm test green, no console.* or secrets in code, then commit. Hosting is Render (or an always-on Replit Reserved VM so RevenueCat webhooks deliver reliably).